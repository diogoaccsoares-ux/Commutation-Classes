...
class Grafo:
        ...
        while(k<len(self.conj_classes)):    
            ...
            for pl in cl.conj_palavras:
                for i in range(2,dim_repr):
                    ...
                    if(pl[i]==pl[i-2] and pl[i-1]==pl[i-3] and abs(pl[i]-pl[i-1])==1 and (pl[i]==0 or pl[i-1]==0) and i>2): #relações longas do tipo 1
    ...
    def Raio(self):
        ...
        s=self.conj_classes[0].representante
        for cl in self.conj_classes:
    ...
    def Diametro(self):
        ...
        print(self.conj_classes[0].representante)
        print("Excentrecidade e distancia: ",self.conj_classes[0].excentrecidade, self.conj_classes[self.conj_classes[0].excentrecidade].representante, self.conj_classes[0].distancia_excentrecidade, "\nDiametro: ",self.diametro, "\nRaio: ", self.raio)
...
def colorir_relacoes_longas(grafo,cv,root):
    palete={3:"orange",4:"green",5:"blue",6:"purple"}
    clear_screen(grafo,cv,root)
    #legenda
...
def main():
    global dim_repr
    a=[0,1,0,2,1,0,3,2,1,0,3,2,1,3,2,3] #Insert a reduced word for some element of type A or B
    ...
    coresmenu.add_command(label="Colorir por soma do tipo A", command=lambda:colorir_soma(G,c,root))
    coresmenu.add_command(label="Colorir por nivel", command=lambda:colorir_soma_2(G,c,root))
    coresmenu.add_command(label="Colorir por número de relações longas", command=lambda:colorir_relacoes_longas(G,c,root))
    #coresmenu.add_command(label="Colorir por conteúdo", command=lambda:colorir_conteudo(G,c,root))
